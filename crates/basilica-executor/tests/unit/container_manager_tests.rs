...
use basilica_executor::config::{ContainerResourceLimits, DockerConfig};
use basilica_executor::container_manager::{
...
impl basilica_executor::container_manager::ContainerManager for MockContainerManager {
    async fn create_container(&self, spec: &ContainerSpec) -> anyhow::Result<String> {
        ...
        let info = ContainerInfo {
            ...
            status: basilica_executor::container_manager::types::ContainerStatus::Created,
    ...
    async fn start_container(&self, container_id: &str) -> anyhow::Result<()> {
        ...
        if let Some(container) = containers.iter_mut().find(|c| c.id == container_id) {
            container.status = basilica_executor::container_manager::types::ContainerStatus::Running;
    ...
    async fn stop_container(&self, container_id: &str, timeout_secs: u64) -> anyhow::Result<()> {
        let mut containers = self.containers.write().await;
        if let Some(container) = containers.iter_mut().find(|c| c.id == container_id) {
            container.status = basilica_executor::container_manager::types::ContainerStatus::Stopped;
    ...
    async fn get_container_logs(
        ...
        Ok(vec![LogEntry {
            ...
            level: basilica_executor::container_manager::types::LogLevel::Info,
    ...
    async fn cleanup_stopped_containers(&self, older_than_hours: u64) -> anyhow::Result<u32> {
        ...
        containers.retain(|c| {
            !(c.status == basilica_executor::container_manager::types::ContainerStatus::Stopped
...
async fn test_mock_container_manager_create() {
    ...
    assert_eq!(
        containers[0].status,
        basilica_executor::container_manager::types::ContainerStatus::Created
    );
}
...
async fn test_mock_container_manager_start() {
    ...
    assert_eq!(
        info.status,
        basilica_executor::container_manager::types::ContainerStatus::Running
    );
...
async fn test_mock_container_manager_stop() {
    ...
    assert_eq!(
        info.status,
        basilica_executor::container_manager::types::ContainerStatus::Stopped
    );
...
async fn test_mock_container_manager_cleanup() {
    ...
    let info = ContainerInfo {
        ...
        name: "old".to_string(),
        image: "alpine:latest".to_string(),
        status: basilica_executor::container_manager::types::ContainerStatus::Stopped,
...
async fn test_container_spec_with_resource_limits() {
    let limits = basilica_executor::container_manager::types::ResourceLimits {
        memory_mb: 2048,
        cpu_cores: 1.5,
        disk_gb: 10,
        gpu_count: 1,
...
async fn test_docker_container_manager_creation() {
    let config = basilica_executor::config::DockerConfig::default();
...
fn test_container_status_display() {
    use basilica_executor::container_manager::types::ContainerStatus as NewContainerStatus;
...
fn test_log_level_display() {
    use basilica_executor::container_manager::types::LogLevel as NewLogLevel;
